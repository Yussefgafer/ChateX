...
class PerformanceSurgicalTest {
    ...
    fun testCircularBufferDeduplication() {
        ...
        // Fill cache
        for (i in 0..9) {
            val p = Packet(id = "p$i", senderId = "s", senderName = "s", type = PacketType.CHAT, payload = "test")
            engine.processIncomingJson("link", gson.toJson(p))
        }
        assertEquals(10, received.size)

        // Try duplicate
        val dup = Packet(id = "p5", senderId = "s", senderName = "s", type = PacketType.CHAT, payload = "test")
        engine.processIncomingJson("link", gson.toJson(dup))
        assertEquals(10, received.size)

        // Evict p0 by adding p10
        val p10 = Packet(id = "p10", senderId = "s", senderName = "s", type = PacketType.CHAT, payload = "test")
